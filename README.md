...
**Static linter for AI agent configs, system prompts, and tool definitions. 7 structural detectors (H1–H7), 6 HERM v1.1 scoring dimensions, validated against 28 comparison files. 156 tests (including a CI-mechanical doc-consistency gate), 0 LLM calls per scan, ~2ms per file.** Reproduce: `bash evals/sample-detection-rate.sh` flags 4-of-4 known-bad samples and passes 1-of-1 clean — same input, same output, every run.
...
makes autonomous AI agents dependable. Founded 2025 by Rolando 
...
(Linguistically Persistent Cognitive Interface) — a language scaffold 
carries state across a stateless model (recall A/B 0.83 vs 0.00 across 
74 sessions, ~2.5x compression; langquant dd918cc). The engineering 
follow-on: when language is the substrate, 
...
- LPCI thesis (recall A/B 0.83 vs 0.00, n=74, ~2.5x compression): 
  langquant repo, commit dd918cc (2026-03-28) + lpci_rigorous.py:507-571
