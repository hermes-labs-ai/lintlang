...
- Dev Container Feature published as
  `ghcr.io/hermes-labs-ai/lintlang/lintlang:1`. It installs one exact LintLang
  release (default `0.6.0`, set through the `version` option) in an isolated
  virtual environment and exposes `lintlang` at `/usr/local/bin/lintlang`.
  Debian and Ubuntu base images on `amd64` and `arm64` are supported; other
  distributions, other architectures, non-root installation, and a `version`
  that is not a valid PEP 440 string stop the build with an explicit message.
- Agent Plugins 1.0 manifest at the Claude Code plugin root
  (`integrations/claude-code/plugin.json`), with the same name and version as
  the existing `.claude-plugin/plugin.json`. It adds a second manifest format
  only; the plugin's hook and skill are unchanged.

### Changed

- Terminal scans end with one dim line naming the installed version and the
  repository URL. It is printed once per scan and only when standard output
  is a TTY; JSON, Markdown, and SARIF output, and piped or redirected terminal
  output, do not contain it.
- `lintlang init --github` generates a workflow pinned to the `v0.6.0` Action
  commit instead of `v0.5.3`. A workflow generated by an earlier release now
  differs from the template, so `init --github` reports the difference and
  leaves the file untouched unless `--force` is supplied.
- The MegaLinter plugin descriptor installs `lintlang==0.7.0` instead of
  `0.5.3`.
...
- **Verdict change.** H2 no longer reports `loop over` or `loop through` as an
  unbounded loop (CRITICAL) unless the same clause also carries an explicit
  indefinite-continuation signal such as `indefinitely` or `forever`, and it
  no longer reports a negated traversal, including an adverb-modified one
  such as `Don't continuously loop over the queue indefinitely.` H4 no longer
  reports `always keep`, `always maintain`, or `always remember` as
  persistence without scope (HIGH) unless the same clause names context,
  memory, history, prior state, or cross-task carryover, so a domain invariant
  such as `Always maintain backward compatibility.` is not reported. Both
  findings produce a FAIL verdict, so a file whose only CRITICAL or HIGH
  findings were these false positives now scans REVIEW or PASS.
  `Loop over the queue indefinitely.`, a `loop until` with no stated bound,
  and `Always remember the conversation history.` are still reported.
- **Exit-code change.** A directory scan that finds no matching files prints
  `No matching files found to scan.` and exits 0. It previously printed
  `Error: No files were successfully scanned.` and exited 1, which failed CI
  on a valid directory that simply held nothing to lint. Missing paths and
  unparseable inputs remain fatal input errors, and `--write-baseline` on an
  empty scan still exits 1 and writes no file, as in 0.6.0. The same change
  adds a warning on standard error when every `--patterns` value is one of H1,
  H3, or H7 and a Python file was scanned: those detectors never run on
  prompts extracted from `.py` sources, so such a scan reported zero
  structural findings regardless of content and looked like a clean PASS. The
  warning changes no exit code and no JSON, Markdown, or SARIF output.
- A tool in the direct format (`name` at the top level of the tool entry)
  whose `name` is not a string is reported as an input error that names the
  entry and the type found, for example
  `tools[0].name must be a string, got integer`, instead of surfacing an
  internal `AttributeError` message. The verdict remains ERROR and the exit
  code remains 1. The OpenAI `function` form is not covered by this fix.
