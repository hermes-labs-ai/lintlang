...
- Terminal scans end with one dim line naming the installed version and the
  repository URL. It is printed once per scan and only when standard output is
  a TTY; JSON, Markdown, and SARIF output, and piped or redirected terminal
...
- The MegaLinter plugin descriptor installs `lintlang==0.6.0` instead of
  `0.5.3`.
...
- **Exit-code change.** A directory scan that finds no matching files prints
  ...
  prompts extracted from `.py` sources, so such a scan reported zero structural
  findings regardless of content and looked like a clean PASS. The warning
  changes no exit code and no JSON, Markdown, or SARIF output.
